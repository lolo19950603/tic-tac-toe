...
const winning_strategies = [
    [[1,2],[3,6],[4,8]], [[0,2],[4,7]], [[0,1],[5,8],[4,6]],
    [[0,6],[4,5]], [[1,7],[3,5],[0,8],[2,6]], [[2,8],[3,4]],
    [[0,3],[4,2],[7,8]], [[6,8],[1,4]], [[6,7],[2,5],[0,4]]
]
...
function nextMove() {
    if (current_move === 'X') {
        if (checkWin() === true) {
            ...
            instruction.innerHTML = 'Player X WIN!&nbsp;&nbsp;&nbsp;<button type="button">RESTART</button>'
            game_is_finished = true;
        } else {
            if (move_count === 9) {
                console.log(instruction.textContent)
                // create restart button
                instruction.innerHTML = 'TIE!&nbsp;&nbsp;&nbsp;<button type="button">RESTART</button>'
                game_is_finished = true;
            } else {
                current_move = 'O'
        ...
        if (checkWin() === true) {
            // create restart button plus dont allow anymore entry
            instruction.innerHTML = 'Player O WIN!&nbsp;&nbsp;&nbsp;<button type="button">RESTART</button>'
            game_is_finished = true;
        } else {
            if (move_count === 9) {
                console.log(instruction.textContent)
...
function checkWin() {
    ...
    if (move_count < 3) {
        ...
        winning_strategy.forEach(function(combo) {
            if (game_pos_status[combo[0]] === current_move && game_pos_status[combo[1]] === current_move) {
                result = true;
                return result;
            }
        })
...
function handleClickPosition(evt) {
    ...
    if (game_is_finished === false) {
        ...
        instruction.innerHTML = 'Invalid entry! Game is finished!&nbsp;&nbsp;&nbsp;<button type="button">RESTART</button>'
    }
  }
